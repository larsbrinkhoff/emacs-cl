  ...
  (let ((nicknames nil)
	...
	(cond
	  ...
	  ((eq keyword (kw SHADOWING-IMPORT-FROM))
	   (let ((p (FIND-PACKAGE (second option))))
	     (dolist (i (cddr option))
	       (push (FIND-SYMBOL (STRING i) p) shadowing-import))))
	  ((eq keyword (kw IMPORT-FROM))
	   (let ((p (FIND-PACKAGE (second option))))
	     (dolist (i (cddr option))
	       (push (FIND-SYMBOL (STRING i) p) import-list))))
	...
	(LET ((,package (MAKE-PACKAGE ,name ,(kw NICKNAMES) ,nicknames)))
	  ...
	  ,@(when shadowing-import
	      `((SHADOWING-IMPORT (QUOTE ,shadowing-import) ,package)))
	  ,@(when use-list
	      `((USE-PACKAGE (QUOTE ,use-list) ,package)))
	  ,@(when import-list
